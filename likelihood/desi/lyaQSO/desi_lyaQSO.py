...
class DESIy1lyaQSOLikelihood(GaussianLikelihood):
    
    data_type = "lyaQSO"
    like_name = "desi_lyaQSO"    

    def __init__(self, options):
        
        super(DESIy1lyaQSOLikelihood, self).__init__(options)
    ...
    def build_data(self):
        ...
        if not self.mode:
            print("DESI Ly-alpha QSO data")
            print("BAO only: Dm(z)/rd and Dh(z)/rd")
            # Reading data file
            DATA_file = os.path.join(ROOT_dir, "desi_lyaQSO_BAO_DMDH.txt")
        else:
    ...
    def build_covariance(self):
        ...
        if not self.mode:
            ...
            COV_file = os.path.join(ROOT_dir, 'desi_lyaQSO_BAO_DMDH_covtot.txt')
        else:
            print("ERROR mode must be 0")
            # Reading covariance matrix file
...
setup, execute, cleanup = DESIy1lyaQSOLikelihood.build_module()
