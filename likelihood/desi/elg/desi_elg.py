...
default_rd_fiducial = 147.09

ROOT_dir = os.path.split(os.path.abspath(__file__))[0]


class DESIy1ELGLikelihood(GaussianLikelihood):
    ...
    def __init__(self, options):
        ...
        super(DESIy1ELGLikelihood, self).__init__(options)
        ...
        self.rd_prior = self.options.get_int("rd_prior", default=0)
    ...
    def build_data(self):
        ...
        if self.rd_prior:
            ...
            print('PRIOR ON rd')
            # Reading data file
            DATA_file = os.path.join(ROOT_dir, "desi_elg_BAO_DMDH_rdprior.txt")
        else:
            if not self.mode:
                print("DESI ELG data")
                print("BAO only: Dm(z)/rd and Dh(z)/rd")
                # Reading data file
                DATA_file = os.path.join(ROOT_dir, "desi_elg_BAO_DMDH.txt")
            else:
                print("ERROR mode must be 0")
    ...
    def build_covariance(self):
        ...
        if self.rd_prior:
            ...
            COV_file = os.path.join(ROOT_dir, 'desi_elg_BAO_DMDH_covtot_rdprior.txt')
        else:
            if not self.mode:
                # Reading covariance matrix file
                COV_file = os.path.join(ROOT_dir, 'desi_elg_BAO_DMDH_covtot.txt')
            else:
                print("ERROR mode must be 0")
                # Reading covariance matrix file
    ...
    def extract_theory_points(self,block):   
        ...
        if self.feedback:
            ...
            if self.rd_prior:
                print('rd prior:  %.3f   %.3f' % (rd, self.data_y[2]))
            if self.mode:
...
setup, execute, cleanup = DESIy1ELGLikelihood.build_module()
