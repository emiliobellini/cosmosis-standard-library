...
class DESIy1LRG_ELGLikelihood(GaussianLikelihood):
    
    data_type = "LRG_ELG"
    like_name = "desi_lrg_elg"    

    def __init__(self, options):
        
        super(DESIy1LRG_ELGLikelihood, self).__init__(options)
    ...
    def build_data(self):
        ...
        if not self.mode:
            print("DESI LRG + ELG data")
            print("BAO only: Dm(z)/rd and Dh(z)/rd")
            # Reading data file
            DATA_file = os.path.join(ROOT_dir, "desi_LRG_ELG_BAO_DMDH.txt")
        else:
    ...
    def build_covariance(self):
        ...
        if not self.mode:
            ...
            COV_file = os.path.join(ROOT_dir, 'desi_LRG_ELG_BAO_DMDH_covtot.txt')
        else:
            print("ERROR mode must be 0")
            # Reading covariance matrix file
...
setup, execute, cleanup = DESIy1LRG_ELGLikelihood.build_module()
