...
class DESIQSOLikelihood(GaussianLikelihood):
	
	data_type = "QSO"
	like_name = "desi_QSO"
	def __init__(self, options):
		
		super(DESIQSOLikelihood, self).__init__(options)
	...
	def build_data(self):
		...
		print("BAO: Dv(z)")
		# Reading data file
		DATA_file = os.path.join(ROOT_dir, "desi_QSO_BAO_DV.txt")
	...
	def build_covariance(self):
		...
		COV_file = os.path.join(ROOT_dir, 'desi_QSO_BAO_DV_covtot.txt')
	...
	def extract_theory_points(self, block):
		...
		Dv_z_rd = [interp(z_eff[0], z, Dv)/rd, interp(z_eff[1], z, Dv)/rd]
		...
		if self.feedback:
			...
			print('BGS Dv_over_rd: %.3f  %.3f  %.3f' % (self.data_x[0], Dv_z_rd[0], self.data_y[0]))
			print('QSO Dv_over_rd: %.3f  %.3f  %.3f' % (self.data_x[1], Dv_z_rd[1], self.data_y[1]))
			print()
			
		return params
		
setup, execute, cleanup = DESIQSOLikelihood.build_module()
